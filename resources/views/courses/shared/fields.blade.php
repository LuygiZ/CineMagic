@php
    ...
    $readonly = $mode == 'show';
@endphp
<x-field.input name="abbreviation" label="Abbreviation" width="md" :readonly="$readonly || $mode == 'edit'"
    value="{{ old('abbreviation', $course->abbreviation) }}" />

<x-field.radio-group name="type" label="Type of course" :readonly="$readonly" value="{{ old('type', $course->type) }}"
    :options="[
        'Degree' => 'Degree',
        'Master' => 'Master',
        'TESP' => 'TESP',
...
<x-field.input name="name" label="Name" :readonly="$readonly" value="{{ old('name', $course->name) }}" />
...
<div class="flex space-x-4">
    ...
    <x-field.input name="semesters" label="Nº Semesters" width="sm" :readonly="$readonly"
...
<x-field.input name="contact" label="Contact" :readonly="$readonly" value="{{ old('contact', $course->contact) }}" />

<x-field.text-area name="objectives" label="Objectives" :readonly="$readonly"
    value="{{ old('objectives', $course->objectives) }}" />

<x-field.text-area name="objectives_pt" label="Objectives (Portuguese)" :readonly="$readonly"
